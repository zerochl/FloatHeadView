...
//      boolean hasStickyHeader = mHeaderPositionCalculator.hasStickyHeader(header, mOrientationProvider.getOrientation(parent), position);
            View header = mHeaderProvider.getHeader(parent, position);
            //每次Draw仅For第一次会做处理，第一项一定是第一个可见的Itemview
            if(isFirstItem){
                isFirstItem = false;
                //只有当第一个HeadView与当前HeadView不同时需要更改，同时调用回调更新UI
                if(currentHeadView != header){
                    currentHeadView = header;
                    onHeadViewChangeListener.onHeadViewChangeListener(currentHeadView);
                }
            }
            boolean hasStickyHeader = mHeaderPositionCalculator.hasStickyHeader(itemView, header, mOrientationProvider.getOrientation(parent), position);
...
//        View header = mHeaderProvider.getHeader(parent, position);
    ...
    public interface OnHeadViewChangeListener {
        void onHeadViewChangeListener(View headView);
    }
}
