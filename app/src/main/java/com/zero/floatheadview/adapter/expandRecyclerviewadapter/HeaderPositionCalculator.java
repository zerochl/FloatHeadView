package com.zero.floatheadview.adapter.expandRecyclerviewadapter;
...
public class HeaderPositionCalculator {
   ...
   */
  public boolean hasStickyHeader(View itemView,View headView, int orientation, int position) {
      int offset = 0, margin = 0;
      mDimensionCalculator.initMargins(mTempRect1, itemView);
      if (orientation == LinearLayout.VERTICAL) {
          offset = itemView.getTop();
          margin = mTempRect1.top + headView.getMeasuredHeight() > itemView.getMeasuredHeight() ? headView.getMeasuredHeight() - itemView.getMeasuredHeight() : 0;
      } else {
          offset = itemView.getLeft();
          margin = mTempRect1.left + headView.getMeasuredWidth() > itemView.getMeasuredWidth() ? headView.getMeasuredWidth() - itemView.getMeasuredWidth() : 0;
      }
...
//      int shiftFromNextHead = translateHeaderWithNextHeader(recyclerView, mOrientationProvider.getOrientation(recyclerView), bounds,
//          header, viewAfterNextHeader, secondHeader);
//        if(isStickyHeaderBeenPushedOffscreen(recyclerView, header,viewAfterNextHeader,shiftFromNextHead)){
//            Log.e("HongLi","has change:" + ((TextView)((LinearLayout)secondHeader).getChildAt(0)).getText().toString());
//        }
//        if(viewAfterNextHeader.getTop() - secondHeader.getMeasuredHeight() < 5){
//            Log.e("HongLi","has change:" + ((TextView)((LinearLayout)secondHeader).getChildAt(0)).getText().toString());
//        }
  ...
  private boolean isStickyHeaderBeingPushedOffscreen(RecyclerView recyclerView, View stickyHeader) {
    ...
    private boolean isStickyHeaderBeenPushedOffscreen(RecyclerView recyclerView, View stickyHeader,View viewAfterHeader,int shiftFromNextHead) {
        int firstViewUnderHeaderPosition = recyclerView.getChildAdapterPosition(viewAfterHeader);
        if (firstViewUnderHeaderPosition == RecyclerView.NO_POSITION) {
            return false;
        }
        ...
        if (firstViewUnderHeaderPosition > 0) {
            ...
            if (mOrientationProvider.getOrientation(recyclerView) == LinearLayoutManager.VERTICAL) {
                int topOfNextHeader = viewAfterHeader.getTop() - mTempRect1.bottom - nextHeader.getHeight() - mTempRect1.top;
//                int bottomOfThisHeader = recyclerView.getPaddingTop() + stickyHeader.getBottom() + mTempRect2.top + mTempRect2.bottom;
                if (topOfNextHeader + shiftFromNextHead <= 5) {
//                    Log.e("HongLi","topOfNextHeader:" + topOfNextHeader);
                    return true;
                }
            } else {
                int leftOfNextHeader = viewAfterHeader.getLeft() - mTempRect1.right - nextHeader.getWidth() - mTempRect1.left;
  ...
  private int translateHeaderWithNextHeader(RecyclerView recyclerView, int orientation, Rect translation,
                                             View currentHeader, View viewAfterNextHeader, View nextHeader) {
    mDimensionCalculator.initMargins(mTempRect1, nextHeader);
    mDimensionCalculator.initMargins(mTempRect2, currentHeader);
      int shiftFromNextHeader = 0;
    if (orientation == LinearLayoutManager.VERTICAL) {
      int topOfStickyHeader = getListTop(recyclerView) + mTempRect2.top + mTempRect2.bottom;
      shiftFromNextHeader = viewAfterNextHeader.getTop() - nextHeader.getHeight() - mTempRect1.bottom - mTempRect1.top - currentHeader.getHeight() - topOfStickyHeader;
      ...
      shiftFromNextHeader = viewAfterNextHeader.getLeft() - nextHeader.getWidth() - mTempRect1.right - mTempRect1.left - currentHeader.getWidth() - leftOfStickyHeader;
      if (shiftFromNextHeader < leftOfStickyHeader) {
        translation.left += shiftFromNextHeader;
      }
    }
      return shiftFromNextHeader;
  }
   ...
   */
  public View getFirstViewUnobscuredByHeader(RecyclerView parent, View firstHeader) {
